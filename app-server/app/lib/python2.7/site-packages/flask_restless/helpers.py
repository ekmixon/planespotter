...
def get_relations(model):
    ...
    return [
        ...
        and k not in RELATION_BLACKLIST
...
def get_field_type(model, fieldname):
    ...
    if not hasattr(field, 'property'):
        return None
    prop = field.property
    return None if isinstance(prop, RelProperty) else prop.columns[0].type
...
def is_date_field(model, fieldname):
    ...
    return isinstance(fieldtype, (Date, DateTime))
...
def get_or_create(session, model, attrs):
    ...
    if all(k in attrs for k in pk_names):
        ...
        pk_values = {k: v for (k, v) in attrs.items() if k in pk_names}
...
class _Singleton(type):
    ...
    def __call__(self, *args, **kwargs):
        ...
        if self not in self._instances:
            supercls = super(_Singleton, self)
            self._instances[self] = supercls.__call__(*args, **kwargs)
